...
int isTmax(int x)
{
  // can use this,  because -1 has the same effect!
  // so must add a condition that x can not be -1!
  // 抓住最大正数和最小负数的特性：+1和取反的效果是一样的
  int a = x + 1;
  int b = ~x;
  return (!(a ^ b)) & (!!(x ^ ~0));
}
